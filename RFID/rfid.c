...
//flags and hilos
           //flag que se utiliza en SPI para saber que una transferencia se ha completado
#define QUEUE_MAX										10                //maximo numero de elementos en la cola
//RFID
#define RFID_READID 								0x02
//FLAGS

#define RFID_STOP                   0x04
#define RFID_START                  0x08
#define TRANSFER_COMRFID				  	0x10
		...
		flags = osThreadFlagsWait(RFID_READID, osFlagsWaitAny, osWaitForever);
			if (flags == RFID_READID) {
				...
				if (status == MI_OK) {
					...
					/*------------TEST---------*/
					sprintf(localObject.buf,"%02X %02X %02X %02X",msg.id[0], msg.id[1], msg.id[2], msg.id[3]);
					localObject.nLin = 1;
					osMessageQueuePut(getModLCDQueueID(), &localObject, 0U, 0U);
					/*-------------------------*/
					/*-------PRINCIPAL-----*/
					//osMessageQueuePut(mid_MsgQueueMFRC522 , &msg, 0U, 0U); --> PARA LA INTEGRACION CON EL RESTO DEL SISTEMA
					/*---------------------*/
...
static void SPI_callback(uint32_t event){
    ...
    case ARM_SPI_EVENT_TRANSFER_COMPLETE:
        osThreadFlagsSet(TID_RC522, TRANSFER_COMRFID);
        break;
...
static void TM_MFRC522_WriteRegister(uint8_t addr, uint8_t val) {
	...
	osThreadFlagsWait(TRANSFER_COMRFID, osFlagsWaitAny, osWaitForever);
	//CS high
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, GPIO_PIN_SET);
}
...
osMessageQueueId_t getMsgRFID(void){
	return mid_MsgQueueMFRC522;
}
...
void det_rfid_on(void) {
  osTimerStart(id_timer_medidas, 100);
}
...
osThreadId_t getModRfidID(void){
	return TID_RC522;
}
...
/*--------------------------MODULO TEST----------------------*/
void Th_RFID_Test(void *argument); 
osThreadId_t tid_Th_RFID_Test;

void Init_Th_RFID_Test(void) {
 	mid_MsgQueueMFRC522 = osMessageQueueNew(QUEUE_MAX, sizeof(MSGQUEUE_RC522_t), NULL);
	tid_Th_RFID_Test = osThreadNew(Th_RFID_Test, NULL, NULL); 
	if (tid_Th_RFID_Test == NULL) {
   // return(NULL);
  }
}
 
static void Th_RFID_Test (void *argument) {
	static uint32_t flags = 0;
	static MSGQUEUE_RC522_t msg;
	static uint8_t CardID[5];
  static TM_MFRC522_Status_t status;
		...
		flags = osThreadFlagsWait(RFID_READID, osFlagsWaitAny, osWaitForever);
			if (flags == RFID_READID) {
				...
				if (status == MI_OK) {
					...
					localObject.nLin = 1;
					osMessageQueuePut(getModLCDQueueID(), &localObject, 0U, 0U);
					//osMessageQueuePut(mid_MsgQueueMFRC522 , &msg, 0U, 0U);
        } else if (status == MI_ERR) {
          //ERROR GESTIONAR
        }
    }
