...
static osMessageQueueId_t mid_MsgQueue_rx,mid_MsgQueue_tx;
static osMessageQueueId_t mid_id_events;
...
void Init_ThFlash (void) {
  //const static osThreadAttr_t th_attr = {.stack_size = 7000};
	...
	mid_id_events=osMessageQueueNew(QUEUE_MAX, sizeof(MSGQUEUE_GEN_PASS_t), NULL);
...
static void Th_flash (void *argument) {
  ...
  while (1) {
    ...
    if (flags & ADD_EVENT) {
      osMessageQueueGet(mid_MsgQueue_rx, &read_event, NULL, osWaitForever);
      addEvent(&read_event);
			error = osThreadFlagsSet(getThIDPrinAccesoManual(), DONE);
    }
...
void add_event1(void) {
  ...
  strcpy (event.description, description);
	...
	osMessageQueuePut(mid_id_events, &event, NULL, osWaitForever);
...
void getAllUsers (void) {
	...
	for (uint8_t usr = 1; usr <= MAX_USERS; usr ++) {
		...
		if (memcmp (user_data, empty_user, US_SIZE_B) != 0) {	//if they are different
			...
			osMessageQueuePut (mid_MsgQueue_tx, &user, NULL, osWaitForever);
			numUsersProccessed++;
		}
...
void getAllEvents (void) {
	...
	for (uint8_t evnt = 1; evnt <= MAX_EVNTS; evnt ++) {
		...
		if (memcmp (event_data, empty_event, EVNT_SIZE_B) != 0) {	//if they are different
			...
			osMessageQueuePut (mid_MsgQueue_tx, &event, NULL, osWaitForever);
			numEventsProccessed++;
		}
