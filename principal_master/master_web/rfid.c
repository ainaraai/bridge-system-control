...
osMessageQueueId_t Init_ThMFRC522 (void) {
	mid_MsgQueueMFRC522 = osMessageQueueNew(QUEUE_MAX, sizeof(MSGQUEUE_RC522_t), NULL);
	TID_RC522 = osThreadNew(Th_rfid, NULL, &threadPH_attr); 
	if (TID_RC522 == NULL) {
    return(NULL);
  }
...
//inicializacion
  ...
  while (1) {
        ...
        //  osThreadFlagsSet(getThIDPrinAccesoManual(), RFID_PRINC);
...
void det_rfid_on(void) {
  osTimerStart(id_timer_medidas, 800);
}
