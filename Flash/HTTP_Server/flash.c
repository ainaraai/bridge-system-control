...
extern osThreadId_t tid_Thread_principal;						//main thread id
static osMessageQueueId_t mid_MsgQueueW25Q16;
static osMessageQueueId_t mid_id2;
...
static void bytesToEvent(const uint8_t *event_data, MSGQUEUE_EVENT_t *event);
static void eventToBytes(const MSGQUEUE_EVENT_t *event, uint8_t *event_data);

void addEvent (MSGQUEUE_EVENT_t *event);
...
osMessageQueueId_t getMsgFlashID(void){
		return mid_MsgQueueW25Q16;
}
...
void Init_ThFlash (void) {
  //const static osThreadAttr_t th_attr = {.stack_size = 7000};
	mid_MsgQueueW25Q16 = osMessageQueueNew(QUEUE_MAX, sizeof(MSGQUEUE_USER_t), NULL);
	mid_id2=osMessageQueueNew(QUEUE_MAX, sizeof(MSGQUEUE_GEN_PASS_t), NULL);
...
static void Th_flash (void *argument) {
  ...
  static MSGQUEUE_USER_t read_user;
  static MSGQUEUE_EVENT_t read_event;
  ...
  while (1) {
    ...
    if(error == osFlagsErrorUnknown) {
      ...
      __breakpoint(0);
		...
		flags = osThreadFlagsWait(ADD_EVENT | DELETE_EVENTS | GET_ALL_EVENTS | ADD_USER | DELETE_USER | GET_ALL_USERS | GET_USER | FLAG_POWER_DOWN | FLAG_POWER_UP, osFlagsWaitAny, osWaitForever);
		if (flags & ADD_USER) {
				osMessageQueueGet(mid_MsgQueueW25Q16, &read_user, NULL, osWaitForever);
				addUser(&read_user);
			  error = osThreadFlagsSet(tid_Thread_principal, ADD_USER);
		}
    if (flags & DELETE_USER) {
				osMessageQueueGet(mid_MsgQueueW25Q16, &read_user, NULL, osWaitForever);
				deleteUser(read_user.id);
			  error = osThreadFlagsSet(tid_Thread_principal, DELETE_USER);
		}
    if (flags & GET_ALL_USERS) {
			getAllUsers();
			error = osThreadFlagsSet(tid_Thread_principal, GET_ALL_USERS);
		}
    if (flags & ADD_EVENT) {
      osMessageQueueGet(mid_MsgQueueW25Q16, &read_event, NULL, osWaitForever);
      addEvent(&read_event);
			error = osThreadFlagsSet(tid_Thread_principal, ADD_EVENT);
    }
    if (flags & DELETE_EVENTS) {
      deleteAllEvents();
			error = osThreadFlagsSet(tid_Thread_principal, DELETE_EVENTS);
    }
    if (flags & GET_ALL_EVENTS) {
      getAllEvents();
			error = osThreadFlagsSet(tid_Thread_principal, GET_ALL_EVENTS);
    }
		if (flags & GET_USER) {
			osMessageQueueGet(mid_MsgQueueW25Q16, &read_user, NULL, osWaitForever);
			getUser(read_user.id);
			error = osThreadFlagsSet(tid_Thread_principal, GET_USER);

		}
    if (flags & FLAG_POWER_DOWN) {
      W25Q16_PoerDown();
			error = osThreadFlagsSet(tid_Thread_principal, FLAG_POWER_DOWN);
    }
    if (flags & FLAG_POWER_UP) {
      W25Q16_PowerUp();
			error = osThreadFlagsSet(tid_Thread_principal, FLAG_POWER_UP);
    }
	}
}
...
void add_user1(char nombre[],uint8_t password[], uint8_t id0,uint8_t id1,uint8_t id2,uint8_t id3,uint8_t id4) {
  ...
  memcpy(user.password, password, 4);
	...
	osMessageQueuePut(mid_MsgQueueW25Q16, &user, NULL, osWaitForever);
...
void add_event1(void) {
	MSGQUEUE_EVENT_t event;
	char description[19] = "PICOT";
  uint8_t day = 16;
  uint8_t month = 06;
  uint16_t year = 2000;
  uint8_t hour = 21;
  ...
  strcpy (event.description, description);
	...
	osMessageQueuePut(mid_MsgQueueW25Q16, &event, NULL, osWaitForever);
...
// Copy the fields of the MSGQUEUE_EVENT_t structure to the byte array (we need to write bytes)
static void eventToBytes(const MSGQUEUE_EVENT_t *event, uint8_t *event_data) {
...
// Copy the fields from the byte array to the MSGQUEUE_EVENT_t structure (we read bytes)
static void bytesToEvent(const uint8_t *event_data, MSGQUEUE_EVENT_t *event) {
